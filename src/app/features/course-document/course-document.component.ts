...
// YouTube video integration removed.

/* Video mappings removed. */
...
@Component({
  selector: 'app-course-document', standalone: true, imports: [RouterLink],
...
export class CourseDocumentComponent implements OnInit, OnChanges {
  ...
  private extractDocument(source: string): string {
    ...
    document.querySelectorAll('title, script, style, link, header, nav, footer, .java-subnav, .site-footer, .spring-pill, .toc, .d-flex.justify-content-between, a.btn-success').forEach((element) => element.remove());

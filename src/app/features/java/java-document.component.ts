...
export class JavaDocumentComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.http.get(`/docs/java/${this.fileName}`, { responseType: 'text' }).subscribe({
  ...
  private extractDocument(source: string): string {
    ...
    document.querySelectorAll('a[href]').forEach((anchor) => {
      ...
      if (target.endsWith('.html')) {
        ...
        anchor.setAttribute('href', `/java/${route === 'index' ? '' : route}${hash}`);
      } else if (target === '../index.html' || target === './index.html') {
        anchor.setAttribute('href', `/java${hash}`);
      }
    });

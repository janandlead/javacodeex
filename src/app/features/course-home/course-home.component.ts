...
@Component({
  ...
  template: `<section class="course-hero"><div class="container-xl"><p class="eyebrow">{{ category }}</p><h1>{{ title }}</h1><p>{{ description }}</p></div></section><section class="container-xl py-5" aria-label="Course topics"><div class="row g-3">@for (item of items; track item.label) { <div class="col-md-6 col-lg-4"><a class="topic-card" [routerLink]="item.route"><i class="bi bi-book" aria-hidden="true"></i><span>{{ item.label }}</span><i class="bi bi-arrow-right ms-auto" aria-hidden="true"></i></a></div> }</div></section>`,

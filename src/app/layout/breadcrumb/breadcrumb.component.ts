...
@Component({
  ...
  template: `<nav class="modern-breadcrumb" aria-label="Breadcrumb"><ol><li><a routerLink="/" aria-label="Home"><i class="bi bi-house-door-fill"></i><span>Home</span></a></li>@for (item of items; track item.label) { <li class="separator" aria-hidden="true"><i class="bi bi-chevron-right"></i></li><li [class.current]="$last" [attr.aria-current]="$last ? 'page' : null">@if (!$last && item.route) { <a [routerLink]="item.route">{{ item.label }}</a> } @else { <span>{{ item.label }}</span> }</li> }</ol></nav>`,

...
@Component({
  ...
  template: `<a class="skip-link" href="#main-content">Skip to main content</a><app-header /><app-navbar /><main id="main-content" class="main-content"><div class="main-layout" [class.course-layout]="isCourseSidebarRoute" [class.interview-layout]="isInterviewRoute">@if (isCourseSidebarRoute) {<aside class="course-sidebar" [attr.aria-label]="sidebarItem?.label + ' lessons'"><div class="sidebar-heading"><i [class]="sidebarItem?.icon ?? 'bi bi-journal-code'" aria-hidden="true"></i><span>{{ sidebarItem?.label }} tutorial</span></div><nav [attr.aria-label]="sidebarItem?.label + ' lessons'">@for (item of sidebarLessons; track item.route) {<a [routerLink]="item.route" routerLinkActive="active" [routerLinkActiveOptions]="{ exact: true }"><span>{{ item.label }}</span></a>}</nav></aside>} @if (isInterviewRoute) {<aside class="interview-sidebar" aria-label="Interview question topics"><div class="sidebar-heading"><i class="bi bi-patch-question" aria-hidden="true"></i><span>Interview topics</span></div><nav aria-label="Interview question topics">@for (topic of interviewTopLevelTopics; track topic.id) {<a class="interview-topic-link" [class.active]="selectedInterviewTopicId === topic.id" [routerLink]="'/java-technical-interview-questions'" [queryParams]="{ topic: topic.id }"><i [class]="topic.icon" aria-hidden="true"></i><span><strong>{{ topic.name }}</strong><small>{{ topic.questionCount }} questions</small></span></a>@for (child of interviewChildTopics(topic.id); track child.id) {<a class="interview-topic-link child" [class.active]="selectedInterviewTopicId === child.id" [routerLink]="'/java-technical-interview-questions'" [queryParams]="{ topic: child.id }"><i [class]="child.icon" aria-hidden="true"></i><span><strong>{{ child.name }}</strong><small>{{ child.questionCount }} questions</small></span></a>}}</nav></aside>}<div class="route-content"><router-outlet /></div></div><app-hibernate-pager /></main><app-footer />`,
  styles: [`
    .main-content{min-height:60vh}.main-layout{min-height:60vh}.course-layout,.interview-layout{display:grid;grid-template-columns:minmax(14rem,18rem) minmax(0,1fr);gap:1.5rem;align-items:start;max-width:90rem;margin:0 auto;padding:1.5rem 1rem 0}.route-content{min-width:0}.course-sidebar,.interview-sidebar{position:sticky;top:1rem;max-height:calc(100vh - 2rem);overflow:auto;padding:.9rem;border:1px solid #dbeafe;border-radius:1rem;background:#fff;box-shadow:0 .5rem 1.25rem rgba(15,23,42,.06)}.sidebar-heading{display:flex;align-items:center;gap:.55rem;margin:.15rem .4rem .8rem;color:#1e3a8a;font-size:.9rem;font-weight:800;letter-spacing:.04em;text-transform:uppercase}.sidebar-heading i{color:#0891b2;font-size:1.1rem}.course-sidebar nav,.interview-sidebar nav{display:grid;gap:.2rem}.course-sidebar a,.interview-topic-link{display:flex;align-items:flex-start;padding:.55rem .45rem;border-radius:.55rem;color:#475569;font-size:.84rem;line-height:1.35;text-decoration:none}.course-sidebar a:hover,.course-sidebar a:focus-visible,.interview-topic-link:hover,.interview-topic-link:focus-visible{color:#0e7490;background:#ecfeff}.course-sidebar a.active,.interview-topic-link.active{color:#1e3a8a;background:#e0f2fe;font-weight:700;box-shadow:inset 3px 0 #06b6d4}.interview-topic-link{gap:.55rem}.interview-topic-link>i{width:1.35rem;flex:0 0 1.35rem;margin-top:.1rem;text-align:center}.interview-topic-link span{display:flex;min-width:0;flex:1;flex-direction:column;gap:.1rem}.interview-topic-link strong{font-size:.82rem}.interview-topic-link small{color:#64748b;font-size:.7rem}.interview-topic-link.child{margin-left:.7rem;padding-left:.55rem;border-left:2px solid #dbeafe}.interview-topic-link.child strong{font-size:.76rem}@media(max-width:991.98px){.course-layout,.interview-layout{display:block;padding-top:1rem}.course-sidebar,.interview-sidebar{position:relative;top:auto;max-height:17rem;margin:0 auto 1rem}.course-sidebar nav,.interview-sidebar nav{grid-template-columns:repeat(2,minmax(0,1fr));column-gap:.5rem}}@media(max-width:575.98px){.course-layout,.interview-layout{padding-left:0;padding-right:0}.course-sidebar,.interview-sidebar{border-radius:0;border-right:0;border-left:0}.course-sidebar nav,.interview-sidebar nav{grid-template-columns:1fr}}
  `]
})
export class MainLayoutComponent {
  readonly interviewTopLevelTopics = INTERVIEW_TOPICS.filter((topic) => !topic.parentId);
  readonly designPatternLessons = [
  ...
  get sidebarItem() {
    ...
    return MAIN_NAVIGATION.find((item) => item.label === 'Java' && (path === '/java' || path === '/java-tutorial-overview' || path.startsWith('/java/') || javaTopicPaths.has(path)) || item.label === 'Spring Boot' && (path === '/project-structure' || this.matchesCoursePath(path, '/spring-boot')) || item.label === 'Hibernate' && this.matchesCoursePath(path, '/hibernate') || item.label === 'Design Patterns' && this.matchesCoursePath(path, '/design-patterns'));
  }

  get sidebarLessons() { return this.sidebarItem?.label === 'Design Patterns' ? this.designPatternLessons : this.sidebarItem?.children ?? []; }
